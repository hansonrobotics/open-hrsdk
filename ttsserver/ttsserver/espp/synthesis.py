...
def appended_utterance_time_stamps(
    ...
    for i in range(len(selected_inflect_block)):
        utterance_time_stamps.append(
            time_stamps[selected_inflect_block[i][:consecutive_blocks]]
        )
...
def normalize_function(utterance_time_stamps):
    ...
    utterance_time_stamps = (
        utterance_time_stamps - min(utterance_time_stamps)
    ) / normal
    normal_two = 0.5 - 0.01
...
def happy_tensed_cents_for_utterance(start_time_now):
    ...
    for i in range(len(start_time_now)):
        cents.append(
            happy_tensed_inflection_function(normalize_function(start_time_now[i]))
        )
    if len(cents) > 0:
        cents = np.concatenate(cents)
        cents = cents.tolist()
...
def afraid_sox_init(
    speed, depth, number_of_bends, start_time_now, end_time_now, cents, tempo, intensity
):
    patch = Transformer()
...
def sad_sox_init(semitones, gain, cutfreq, tempo):
    patch = Transformer()
    patch.pitch(semitones, False)
    patch.tempo(tempo, "s")
    patch.treble(gain, cutfreq, 0.5)
...
def sad_patch(
    ...
    sad_patch = sad_sox_init(semitones, gain, cutfreq, tempo)
    return sad_patch
